...
namespace Edgeteks.MWLServer
{
    ...
    public class MPPSScp : IDicomServerHandler
    {
        ...
        private static bool IsMppsEntitycached(string key)
        {   
            foreach (int  associationHash in _mppsCache.Keys)
            {
                ListDictionary mppsAssociationList;
                if (_mppsCache.TryGetValue(associationHash, out mppsAssociationList))
        ...
        private static void RemoveMppsEntityFromCache(string key)
        {
            foreach (int associationHash in _mppsCache.Keys)
            {
                ListDictionary mppsAssociationList;
        ...
        private static ModalityPerformedProcedureStepIod GetCachedMppsIod(string cacheKeyId)
        {
            foreach (int associationHash in _mppsCache.Keys)
            {
                ListDictionary mppsAssociationList;
